...
df_rate = pd.read_csv("./ml-latest-small/ratings.csv", sep=",")
#最初の0~2列目までを利用（ユーザーID、映画ID、評価の3列を利用）
df_rate = df_rate.iloc[:,0:3]

df_name = pd.read_csv("./ml-latest-small/movies.csv",sep=",")

#疎行列(行列の中身が0ばかりの行列)に変換。行名が映画のID、列名がユーザーID、行列の中身が評価
df_piv = df_rate.pivot(index= "movieId",columns="userId",values="rating").fillna(0)
...
Movieid = 100#任意の好きな映画ID
distance, indice = rec_model.kneighbors(df_piv.iloc[df_piv.index== Movieid].values.reshape(1,-1),n_neighbors=11)

#print(df_name.columns)
#print(df_name.at[5,'title'])
#print(df_name.at[Movieid, "title"])
for i in range(11):#i=0は映画ID100のことであり、i=1~10までは評価が似ている映画のこと。
    if  i == 0:#i=0はID100のこと。
        print("{}を見た人におすすめの映画は以下です。".format(df_name.at[Movieid, "title"]))
    else:
        print("{}: {}".format(i,df_name.at[df_piv.index[indice.flatten()[i]], 'title']))
